...
import { Save, CheckCircle, AlertCircle, Loader2, Lock, Copy } from "lucide-react";
...
export default function AdminPage() {
  ...
  useEffect(() => {
    // Проверяем, открыто ли это внутри Telegram
    if (typeof window !== "undefined" && window.Telegram?.WebApp) {
      ...
      if (user) {
        ...
        // Если ID нет (открыто в браузере)
  ...
  if (checkingAuth) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 text-gray-400 gap-3">
        ...
        <p>Получение Telegram ID...</p>
      </div>
    );
  }

  // ЭКРАН 2: ДОСТУП ЗАПРЕЩЕН (ЗДЕСЬ МЫ ПОКАЗЫВАЕМ ID)
  if (!isAuth) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4 font-sans">
        <div className="bg-white p-6 rounded-2xl shadow-lg w-full max-w-sm text-center">
          ...
          <p className="text-gray-500 mb-6 text-sm">
            Вашего ID нет в списке администраторов.
          </p>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-6">
            ...
            {userInfo?.id ? (
                <div className="flex items-center justify-center gap-2">
                    <code className="text-2xl font-mono font-bold text-slate-800">
                        {userInfo.id}
                    </code>
                </div>
            ) : (
                <div className="text-red-500 font-medium text-sm">
                    ...
                    <span className="text-xs opacity-75 font-normal">
                        (Откройте через кнопку в Боте, а не в браузере)
                    </span>
                </div>
            )}
          </div>
          ...
          {userInfo?.id && (
            <div className="text-xs text-gray-400 bg-gray-100 p-3 rounded-lg break-all font-mono">
              Добавьте это число в Vercel Env:<br/>
              <span className="text-gray-600">ADMIN_IDS</span>
            </div>
          )}
        </div>
      </div>
